...
if @team_assessments_exists
  ...
  [:female, :male].each do |gender|
    if resource.team_assessment_rows(gender, true).present?
      pdf.start_new_page unless first
      first = false
      pdf_header(pdf, resource.to_s, "Mannschaft #{g(gender)}")
      ...
      resource.team_assessment_rows(gender, true).map(&:decorate).each do |row|
        line = [row.rank, numbered_team_name(row, competition_id: resource.cups.map(&:competition_id), gender: gender)]
        cups.each do |cup|
