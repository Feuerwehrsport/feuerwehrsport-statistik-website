...
class Error
  ...
  click: =>
    if @isOpen
      ...
      unless @openTrs
        code = $('<tr/>').append($('<td/>').attr('colspan', 4).append($('<pre/>').text(
  ...
  getActionBox: (div, callback = null, cols = 3) =>
    box = @box(cols, div).append($('<div/>').addClass('btn btn-default').text('Erledigt').click( =>
      @confirmDone('Ohne Aktion - ')
    ))
    if callback
      box.append('<br/>').append($('<div/>').addClass('btn btn-success').text('Beheben').click( =>
    ...
    switch @key
            ...
            .on('submit', (data) =>
              @confirmAction =>
                Fss.put(
                  ...
                  { competition: data, log_action: 'update-competition:name' },
                  => @confirmDone()
                )
            )
            .open()
      when 'competition-add-hint'
        ...
        @openType = (div) =>
          ...
          .append($('<a/>').attr('href', "/backend/competitions/#{@data.competition_id}/edit").
            text('Wettkampf bearbeiten'))
          hintsBox = @box(3, div)
          @getActionBox(div)
      else
        @openType = (div) =>
    ...
    switch @key
      when 'person-correction'
        ...
        @openType = (div) =>
          ...
          @getActionBox div, =>
            Fss.put(
              "people/#{@data.person_id}",
              { person: @data.person, log_action: 'update-person:name' },
              => @confirmDone()
            )
      ...
      when 'person-change-nation'
        ...
        @openType = (div) =>
          ...
          @getActionBox div, =>
            Fss.put(
              "people/#{@data.person_id}",
              { person: { nation_id: @data.nation_id }, log_action: 'update-person:nation' },
              => @confirmDone()
            )
      else
        @openType = (div) =>
          ...
          $.each @files, (i, file) =>
            showButton = $('<div/>').addClass('btn btn-info').text('Anzeigen').click =>
              ...
              Fss.getResource "change_requests/#{@id}/files", i, (file) ->
                $('<img/>').attr('src', "data:#{file.content_type};base64,#{file.binary}").css('width', '200px').
                  appendTo(imageBox)
                selectButton.show()
            selectButton = $('<div/>').hide().addClass('btn btn-success').text('Auswählen').click =>
              @confirmAction =>
                Fss.put(
                  "teams/#{@data.team_id}",
                  { team: { image_change_request: "#{@id}-#{i}" }, log_action: 'update-team:image' },
                  => @confirmDone()
                )
            imageBox = $('<div/>')
