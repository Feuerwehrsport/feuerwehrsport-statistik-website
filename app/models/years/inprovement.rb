class Years::Inprovement < Struct.new(:year, :discipline, :gender, :team)
  ...
  def all
    items = {}
    Score.send(discipline).year(year).where(person: person_ids).includes(:person).includes(competition: %i[place event]).each do |score|
      items[score.person_id] ||= Years::InprovementItem.new(score.person)
      items[score.person_id].current_scores.push score
    end
    Score.send(discipline).year(last_year).where(person: person_ids).includes(:person).includes(competition: %i[place event]).each do |score|
      items[score.person_id] ||= Years::InprovementItem.new(score.person)
