...
RSpec.describe 'Api::Imports' do
  describe 'POST check_lines' do
    let(:r) { -> { post '/api/imports/check_lines', params: { import: } } }

    let!(:warin) { create(:team) }
    let(:raw_lines) { "Warin;foo;22\nFF Wurststadt;bla;44\nFC Hansa Rostock;blub;D" }
    let(:import) do
      { discipline: 'la', gender: 'male', separator: ';', raw_headline_columns: 'team;col;time', raw_lines: }
    end
