...
RSpec.describe 'Api::GroupScoreTypes' do
  let(:group_score_type) { create(:group_score_type) }

  describe 'POST create' do
    let(:r) do
      -> {
        post '/api/group_score_types', params: { group_score_type: { name: 'Extrapokal', discipline: 'la' } }
      }
    end
  ...
  describe 'GET index' do
    ...
    context 'when discipline given' do
      ...
      it 'returns group_score_categories' do
        get '/api/group_score_types', params: { discipline: 'gs' }
        expect_json_response
        expect(json_body[:group_score_types].first).to eq(
          discipline: 'gs',
          id: group_score_type.id,
