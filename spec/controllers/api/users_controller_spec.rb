...
RSpec.describe API::UsersController, type: :controller do
  describe 'GET status' do
    ...
    context "when session user id is set" do
      it "returns login false" do
        expect(User).to receive(:find_by_id).with(99).and_return(:user)
  ...
  describe 'POST create' do
    it "creates new user and sign in" do
      expect {
        post :create, user: { name: "Benutzer", email_address: "email-address@foo.de" }
    ...
    context "when email_address is not valid" do
      it "fails to create new user" do
        post :create, user: { name: "Benutzer", email_address: "not-valid" }
